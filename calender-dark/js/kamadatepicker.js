...
function kamaDatepicker(elementID, opt) {
    ...
    inputElement.after(
        "<div id='bd-main-" +
        elementID +
        "' class='bd-main bd-hide text-center p-3 bg-gray-800 rounded-lg border-gray-600 mt-2' style='position: absolute; direction: rtl;'></div>" //add class bd-hide
    );
    ...
    calendarDiv.append("<div class='bd-title flex flex-row items-center'></div>");
    var titleDiv = calendarDiv.find('.bd-title');
    calendarDiv.append(
        "<table class='bd-table bg-gray-500 ' dir='rtl' cellspacing='0' cellpadding='0'></table>"
    );
    ...
    if (options.swapNextPrev)
        titleDiv.append(
            "<button id='bd-prev-" +
            elementID +
            "' class='bd-prev basis-1/4 ' type='button' title='ماه قبلی' data-toggle='tooltip'><span>قبلی</span></button>"
        );
    else
        titleDiv.append(
            "<button id='bd-next-" +
            elementID +
            "' class='bd-next basis-1/4 bg-gray-100 rounded-md mx-1 hover:bg-gray-400 hover:text-gray-100' type='button' title='ماه بعدی' data-toggle='tooltip'><span>بعدی</span></button>"
        );

    titleDiv.append(
        "<div class='bd-dropdown my-3 basis-1/4'></div><div class='bd-dropdown my-3 basis-1/4'></div>"
    );
    titleDiv
        .find('.bd-dropdown:nth-child(2)')
        .append(
            "<select id='bd-month-" + elementID + "' class='bd-month bg-gray-600 text-white rounded-r-lg border-l-2 border-gray-500'></select>"
        );
    titleDiv
        .find('.bd-dropdown:nth-child(3)')
        .append("<select id='bd-year-" + elementID + "' class='bd-year bg-gray-600 text-white rounded-l-lg'></select>");

    if (options.swapNextPrev)
        titleDiv.append(
            "<button id='bd-next-" +
            elementID +
            "' class='bd-next basis-1/4' type='button' title='ماه بعدی' data-toggle='tooltip'><span>بعدی</span></button>"
        );
    else
        titleDiv.append(
            "<button id='bd-prev-" +
            elementID +
            "' class='bd-prev basis-1/4 bg-gray-100 rounded-md mx-1 hover:bg-gray-400 hover:text-gray-100' type='button' title='ماه قبلی' data-toggle='tooltip'><span>قبلی</span></button>"
        );
    ...
    calendarDiv
        ...
        .append(
            ...
            elementID +
            "' class='bd-table-days bg-gray-600'></tbody>"
        );
    var daysTable = $('#bd-table-days-' + elementID);

    // create go to todays button
    if (options.gotoToday) {
        calendarDiv.append("<div class='bd-goto-today bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-blue-500 hover:to-cyan-500 rounded-lg mb-2 mt-3'>برو به امروز</div>");
    ...
    function setCalendarPosition() {
        ...
        if (options.position === 'top') {
            ...
            // bottom
                ...
                calendarContainer.style.top = `${-1 * calendarContainer.offsetHeight
                    }px`;
            // top
        ...
        while (dayIndex <= numberOfDays) {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                if (i < 7 && dayIndex <= numberOfDays) {
                    var tempTD =
                        '<td>' +
                        '<button class="rounded-md m-1 day day-' +
                        dayIndex +
                        '" type="button">' +
